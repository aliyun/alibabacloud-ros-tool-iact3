...
class Policy:
    ...
    @classmethod
    async def create(cls, template: str = None, config_file: str = None, regions: str = None):
        ...
        if template:
            ...
            base_config = BaseConfig.create(
                project_config_file=config_file or DEFAULT_CONFIG_FILE, args={PROJECT: args}, project_path=project_path
            )
        ...
        LOG.info(f'start getting policies of the templates.')

        policies = await plugin.generate_template_policy(**template_args)
        TerminalPrinter._display_policies(policies=policies)
