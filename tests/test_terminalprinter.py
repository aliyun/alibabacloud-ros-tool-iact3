...
class TestConfig(BaseTest):
    async def test_display_price(self):
        template_price = {
            "KubernetesCluster": {
                ...
                "Result": {
                    "Order": {"OriginalAmount": 0, "DiscountAmount": 0, "TradeAmount": 0, "Currency": ""},
                    "OrderSupplement": {"PriceUnit": "/Hour", "ChargeType": "PostPaid", "Quantity": 1},
                    "AssociationProducts": {
                        "SlbInstanceApiServer": {
                            "Type": "ALIYUN::SLB::LoadBalancer",
                            "Result": {
                                "Order": {"OriginalAmount": 2, "DiscountAmount": 1, "TradeAmount": 1, "Currency": ""},
                                "OrderSupplement": {"PriceUnit": "/Hour", "ChargeType": "PostPaid", "Quantity": 1},
                            },
                            "AssociationCU": {
                                "OrderSupplement": {
        ...
        self.assertIn(
            "KubernetesCluster  cn-hangzhou  ALIYUN::CS::KubernetesCluster    PostPaid      /Hour                  1                          0                0              0    ",
            logs[3].getMessage(),
        )
        self.assertIn(
            "ALIYUN::SLB::LoadBalancer        PostPaid      /Hour                  1                          2                1              1",
            logs[4].getMessage(),
        )
        self.assertIn(
            "SLB::LoadBalancer-AssociationCU  PostPaid      /CU                    1  CNY                     0.23             0.069          0.161",
            logs[5].getMessage(),
        )
    ...
    async def test_display_price_with_no_template_price(self):
        ...
        stack = Stack(
            stack_name="test-stack", region="cn-hangzhou", stack_id="", template_price=None, status_reason="**"
        )
        stack.status = "*"
        stacker = Stacker('test', tests=[], stacks=[stack])
        TerminalPrinter._display_price(stacker)
    ...
    async def test_display_preview_resources(self):
        preview_result = [
            {
                ...
                "Properties": {"InstanceId": "EcsInstance", "AllocationId": "EIP", "InstanceType": "EcsInstance"},
            }
        ]
        memory_handler = logging.handlers.MemoryHandler(capacity=10240)
        logger = logging.getLogger()
        ...
        logger.addHandler(memory_handler)
        stack = Stack(
            stack_name="test-stack", region="cn-hangzhou", stack_id="", preview_result=None, status_reason="**"
        )
        stack.status = "*"
        stacker = Stacker('test', tests=[], stacks=[stack])
        TerminalPrinter._display_preview_resources(stacker)
